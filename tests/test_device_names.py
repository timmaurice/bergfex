...
from custom_components.bergfex.__init__ import _async_refresh_device_name
...
def _coordinator(entry: MockConfigEntry, data) -> None:
    """Stand in for the resort coordinator, which only its `data` matters here.

    Setup keeps it in the entry's runtime_data, which is where the rename reads it.
    """
    entry.runtime_data = type("Coordinator", (), {"data": data})()
...
async def test_the_slug_is_replaced_by_the_parsed_name(hass: HomeAssistant):
    ...
    _coordinator(entry, {AREA: {"resort_name": "Achensee"}})
...
async def test_a_resort_without_a_parsed_name_keeps_what_it_has(hass: HomeAssistant):
    entry = _entry(hass)
    device = _device(hass, entry, "achensee")
    _coordinator(entry, {AREA: {"snow_mountain": 40}})
...
async def test_a_correct_name_is_left_alone(hass: HomeAssistant):
    ...
    _coordinator(entry, {AREA: {"resort_name": "Sölden"}})
